...
function main(args) {
	...
	for (var i = 0; i < design_elements_num; i++) {
		...
		cutOut.setTransform(new Translation(
			section_size * design[i][0] - curve_length / 2 + bar_width / 2,
			section_size * design[i][1] - height / 2 + bar_width / 2,
			0));
...
function generateTetramino(squares) {
	...
	for (var i = 0; i < squares.length; i++) {
		var square = new Box(
			(squares[i][0] + 0.5) * section_size,
			(squares[i][1] + 0.5) * section_size,
			0,
			square_side,
			square_side,
			thickness * 2
		...
		for (var j = 0; j < squares.length; j++) {
			...
			if (squares[i][0] == squares[j][0] && squares[i][1] - squares[j][1] == 1) {
				...
				tetramino.add(new Box(
					(squares[i][0] + 0.5) * section_size,
					(squares[i][1] + 0.5) * section_size - square_side + bar_width / 2, // one below
					0,
					square_side,
					bar_width,
					thickness * 2
			...
			if (squares[i][1] == squares[j][1] && squares[i][0] - squares[j][0] == 1) {
				...
				tetramino.add(new Box(
					(squares[i][0] + 0.5) * section_size - square_side + bar_width / 2,  // one on the left
					(squares[i][1] + 0.5) * section_size,
					0,
					bar_width,
					square_side,
...
function init() {
	...
	O  = generateTetramino([[0, 0], [0, 1], [1, 0], [1, 1]]); // O (square)
